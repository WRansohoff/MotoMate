...
// 'Amplitude' macro; larger values mean higher volume.
#define _AMP(x) ( x / 8 )
// 32-sample sine wave.
...
int main(void) {
  ...
  #elif VVC_L4
    // STM32L4 DMA has an extra 'channel selection' register
    // to help it handle the larger number of peripherals.
    // TIM6_UP and DMA_CH1 map to channel 3 when C3S is set to 0x6.
    DMA1_CSELR->CSELR &= ~( DMA_CSELR_C3S );
    DMA1_CSELR->CSELR |=  ( 0x6 << DMA_CSELR_C3S_Pos );
  #endif
